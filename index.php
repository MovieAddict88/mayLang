...
<head>
    ...
    <style>
        /* Fix for description stretching and overlapping */
        .video-description, .carousel-content p, .card-info p {
            white-space: normal !important;
            word-wrap: break-word !important;
            overflow-wrap: break-word !important;
            max-width: 100% !important;
        }
        /* Ensure grid cards expand if content is large, or hide overflow properly */
        ...
        /* TV Safe Dropdowns */
        select.filter-select, select.episode-selector, select.server-selector {
        ...
        select:focus {
            outline: 2px solid var(--youtube-red) !important;
...
.content-card.grid .card-meta .meta-country .fi {
    ...
    height: 140px; /* Increased base height */
        ...
        /* Mobile-specific back button styling (smartphones) */
        @media (max-width: 768px) {
            .back-button {
                padding: 14px 24px;
                font-size: 16px;
        ...
        /* Smart TV and large screen styling */
                        ...
                        <div class="video-info">
                            ...
                            <div class="video-meta">
                                ...
                                <div class="video-actions">
                                    <button class="action-btn" id="like-btn"><i class="fas fa-thumbs-up"></i> <span id="like-count-span">24K</span></button>
    ...
    <script>
        ...
        function createContentCard(item, viewType) {
            ...
            card.addEventListener('click', (e) => {
                // Check if the click target is the watch later button or delete button
                if (e.target.closest('.card-watch-later-btn') || e.target.closest('.delete-watch-later-btn')) {
                    return; // Don't play trailer if clicking these buttons
                }
                
                // If item has a trailer, show it first
                if (item.Trailer && item.Trailer.url) {
                    showTrailerOverlay(item);
                } else {
                    openViewer(item);
                }
            });
        ...
        // Show trailer overlay with auto-play
        function showTrailerOverlay(item) {
            ...
            if (!videoId) {
                // If we can't extract video ID, just open viewer
                openViewer(item);
                return;
            }
            ...
            closeBtn.className = 'trailer-close-btn';
            closeBtn.innerHTML = '<i class="fas fa-times"></i>';
            closeBtn.onclick = () => {
                document.body.removeChild(overlay);
            };
            
            // Create watch full button
            const watchBtn = document.createElement('button');
            watchBtn.className = 'trailer-watch-full';
            watchBtn.textContent = 'Watch Full Content';
            watchBtn.onclick = () => {
                document.body.removeChild(overlay);
                openViewer(item);
            };
            
            // Create iframe with autoplay and unmuted
        ...
        function openViewerInternal(content) {
            ...
            window.scrollTo(0, 0);
            const newShareHandler = () => handleShareVideo(content);
            if (elements.shareVideoBtn._shareHandler) {
                elements.shareVideoBtn.removeEventListener('click', elements.shareVideoBtn._shareHandler);
            }
            elements.shareVideoBtn.addEventListener('click', newShareHandler);
            elements.shareVideoBtn._shareHandler = newShareHandler;

            updateLikeDislikeUI(content.Title);

            const newLikeHandler = () => handleLike(content);
            if(elements.likeBtn._likeHandler) elements.likeBtn.removeEventListener('click', elements.likeBtn._likeHandler);
            elements.likeBtn.addEventListener('click', newLikeHandler);
            elements.likeBtn._likeHandler = newLikeHandler;

            const newDislikeHandler = () => handleDislike(content);
            if(elements.dislikeBtn._dislikeHandler) elements.dislikeBtn.removeEventListener('click', elements.dislikeBtn._dislikeHandler);
            elements.dislikeBtn.addEventListener('click', newDislikeHandler);
            elements.dislikeBtn._dislikeHandler = newDislikeHandler;
    ...
    try {
        ...
        function setupMobileBackButton() {
            ...
            if (isMobile) {
                ...
                // Add swipe gesture support for going back
            ...
            // Handle mobile back button (Android) - Fixed to use proper history navigation
            if (navigator.userAgent.includes('Android')) {
                document.addEventListener('backbutton', function(e) {
                    if (elements.viewerPage.style.display !== 'none') {
                        ...
                        // Use history.back() to properly navigate back
                        // The popstate event will handle the actual UI changes
                        history.back();
                    }
                }, false);
            }

            // Handle ESC key for desktop and back button for smart TVs
            document.addEventListener('keydown', function(event) {
                if (elements.viewerPage.style.display !== 'none') {
                    // Handle ESC key (desktop/laptop)
                    if (event.key === 'Escape') {
                        event.preventDefault();
                        console.log('ESC key pressed, navigating back');
                        history.back();
                    }
                    // Handle Backspace key (some TVs and devices)
                    else if (event.key === 'Backspace' && !['INPUT', 'TEXTAREA'].includes(document.activeElement.tagName)) {
                        event.preventDefault();
                        console.log('Backspace key pressed, navigating back');
                        history.back();
                    }
                    // Handle TV remote back button codes
                    else if (event.keyCode === 10009 || event.keyCode === 461 || event.keyCode === 8) {
                        event.preventDefault();
                        console.log('TV remote back button pressed, navigating back');
                        history.back();
                    }
                }
            });
        ...
        // Setup swipe back gesture for mobile devices
        function setupSwipeBackGesture() {
            ...
            const viewerPage = elements.viewerPage;
            
            viewerPage.addEventListener('touchstart', function(e) {
                // Only track if touch starts from the left edge of the screen (for back swipe)
            ...
            viewerPage.addEventListener('touchmove', function(e) {
                if (touchStartX > 0) {
            ...
            viewerPage.addEventListener('touchend', function(e) {
                if (touchStartX === 0) return;
                
                const horizontalDistance = touchEndX - touchStartX;
                const verticalDistance = Math.abs(touchEndY - touchStartY);
                
                // Check if it's a right swipe (going back) from the left edge
        ...
        function setupEventListeners() {
            // TV/Console fix for dropdowns: ensure they are clickable and handle focus better
            const selects = document.querySelectorAll('select');
            selects.forEach(select => {
                select.addEventListener('click', (e) => {
                    // On some TVs, click doesn't always open the menu if not focused first
                    select.focus();
                });
            ...
            // Back button in viewer - Fixed to properly navigate back
            elements.backButton.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                console.log('Back button clicked - navigating back in history');
                // Use history.back() to properly go back instead of pushing new state
                history.back();
            });

            // Enhanced mobile back button support with proper touch handling
            if ('ontouchstart' in window) {
                ...
                elements.backButton.addEventListener('touchend', function(e) {
                    ...
                    if (touchStarted) {
                        ...
                        console.log('Back button touched - navigating back in history');
                ...
                // Handle touch cancel
                elements.backButton.addEventListener('touchcancel', function(e) {
                    this.style.transform = 'scale(1)';
                    touchStarted = false;
                });
                ...
                elements.backButton.addEventListener('touchmove', function(e) {
                    // Only prevent if touch moved too far (allow small movements)
                    if (touchStarted) {
                        e.preventDefault();
                    }
                }, { passive: false });
  ...
  return function(...args) {
    ...
    timeout = setTimeout(later, wait);
        ...
        function updateLikeDislikeUI(contentId) {
            if (!elements.likeBtn || !elements.dislikeBtn) return; // Ensure elements are present

            const interactions = getVideoInteractions(contentId); // Get fresh data
            elements.likeCountSpan.textContent = formatCount(interactions.likes);
            elements.dislikeCountSpan.textContent = formatCount(interactions.dislikes);
            ...
            } else if (interactions.userAction === 'disliked') {
